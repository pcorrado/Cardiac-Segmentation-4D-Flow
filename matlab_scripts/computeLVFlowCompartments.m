function data = computeLVFlowCompartments(inputDir,tShift, maskFile)
    %computeLVFlowCompartments compute the amount of intra-LV flow in each of
%   the 4 main flow compartments (see Eriksson J, et al. 2010. 
%   https://doi.org/10.1186/1532-429X-12-9)
%   data = computeLVFlowCompartments(inputDir) calculate flow compartments 
%   based on 4D flow data and mask located in directory 'srcDir'
...
%   CD.nii, VELX.nii, VELY.nii, VELZ.nii, registeredMask.nii)
    ...
    if nargin>2 && exist(maskFile,'file')
        mask = circshift(round(squeeze(niftiread(maskFile))),[0,0,0,tShift]);
    ...
    % Forward through time mask and velocity
    maskF = mask(:,:,:,1:esvTime);
    vxF = vx(:,:,:,1:esvTime);
    vyF = vy(:,:,:,1:esvTime);
    vzF = vz(:,:,:,1:esvTime);
    % Reverse through time mask and velocity
...
function data = classifyPaths(mask, paths, fPaths, rPaths, voxSize)
    ...
    for ii = 1:nPath
        ...
        % Check that pathline only enters or leaves LV through the base,
        % not the mid-ventricle or the apex.
        qcCheck(ii) = (firstIn(ii)==1  || locations(firstIn(ii),ii)==3) && ...
                      ( lastIn(ii)==nT || locations(lastIn(ii),ii)==3);             %#ok<AGROW>
    end
    startsIn = firstIn==1; 
    enters = firstIn~=1; 
    endsIn = lastIn==nT;
...
function paths = computePaths(mask, vx, vy, vz, voxSize)
    ...
    fprintf('Setting up interpolation grids.\n');
    if (nT>1)
    ...
    fprintf('Found %i mask voxels. Making %i pathlines.\n',nVox,nVox);
    ...
    fprintf('Done computing pathlines.\n');
