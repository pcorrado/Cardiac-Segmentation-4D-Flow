function data = computeRVFlowCompartments(inputDir,tShift, maskFile)
    %computeRVFlowCompartments compute the amount of intra-RV flow in each of
...
%   CD.nii, VELX.nii, VELY.nii, VELZ.nii, registeredMask.nii)
    ...
    if nargin>2 && exist(maskFile,'file')
        mask = circshift(squeeze(round(niftiread(maskFile))),[0,0,0,tShift]);
    ...
    rvVolume = squeeze(sum(sum(sum(mask<0,1),2),3).*vol.*1e6); % in mL
    
    staticMask = false;
    if all(rvVolume==rvVolume(1)); staticMask=true; end
        
    [~,edvTime] = max(rvVolume);
    if staticMask       
        edvTime = 7-tShift;    
    end
